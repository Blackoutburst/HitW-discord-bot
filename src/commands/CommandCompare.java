...
public class CommandCompare extends CommandExecutable {
	...
	private void setSubtitle(Canvas image) {
		if (command.getArgs().length == 1) {
			image.drawStringLeft(Stats.getSubTitle(GeneralUtils.getIGNfromDiscord(command.getSender().getId())), 10, 70, 26, Color.white);
	...
	private void setBackground(Canvas image) {
		...
		if (command.getArgs().length == 1) {
			half.drawImage(GeneralUtils.getCustomBackground(GeneralUtils.getIGNfromDiscord(command.getSender().getId())), 0, 0, 600, 400);
			half.save("half.png");
			...
			half.drawImage(GeneralUtils.getCustomBackground(command.getArgs()[0]), 0, 0, 600, 400);
			half.save("half.png");
			
			image.drawCustomBackground(GeneralUtils.getCustomBackground(command.getArgs()[1]), 0, 0, 600, 400);
			image.drawCustomBackground("half.png", 0, 0, 300, 400);
		}
	}
